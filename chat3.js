...
function displayMessage(message, type = "bot", isInitial = false, isAnswer = false) {
    ...
    if (!isInitial && !isAnswer) {
        localStorage.setItem("therabot_chat_history", chatBody.innerHTML);
    }
...
nextButton.addEventListener("click", () => {
    ...
    displayMessage(userResponse, "user", false, true); // Store user answer
...
displayMessage("Hello, how can I help?", "bot", true);
loadNextQuestion();
...
function loadChatHistory() {
    ...
    if (userAnswers.length > 0) {
        userAnswers.forEach(answer => {
            display.innerHTML += `<p>${answer}</p>`;
        });
    } else {
        display.innerHTML = "<p>No chat history found.</p>";
    }
}

if (document.getElementById("viewChatBtn")) {
    document.getElementById("viewChatBtn").addEventListener("click", loadChatHistory);
}